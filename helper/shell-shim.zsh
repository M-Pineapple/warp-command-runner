#!/usr/bin/env zsh
# claude-command-runner — Tier E shell shim for zsh
#
# When sourced from ~/.zshrc, adds two hooks that emit JSON events to a
# Unix domain socket the MCP server listens on. This gives CCR clean
# block-boundary visibility (preexec / command_finished) without polling
# /tmp/claude_output_<id>.json files.
#
...
typeset -g _ccr_shim_sock="/tmp/ccr-shell-shim-${UID}.sock"
typeset -g _ccr_shim_last_cmd=""
typeset -g _ccr_shim_preexec_ts=""

_ccr_shim_send() {
    # Skip silently if not inside Warp.
    [[ -z "$WARP_SESSION_ID" ]] && return 0
    # Skip silently if the socket is missing (MCP not listening).
    [[ ! -S "$_ccr_shim_sock" ]] && return 0
    ...
    python3 - "$_ccr_shim_sock" "$1" 2>/dev/null <<'PYEOF' &!
import socket, sys
...
_ccr_shim_preexec() {
    _ccr_shim_last_cmd="$1"
    _ccr_shim_preexec_ts="$(date +%s.%N 2>/dev/null || date +%s)"
    local cmd_escaped
    cmd_escaped="$(_ccr_shim_json_escape "$1")"
    local sid_escaped
    sid_escaped="$(_ccr_shim_json_escape "${WARP_SESSION_ID:-}")"
    _ccr_shim_send "{\"type\":\"preexec\",\"command\":\"$cmd_escaped\",\"warp_session_id\":\"$sid_escaped\",\"ts\":\"$_ccr_shim_preexec_ts\"}"
}

_ccr_shim_precmd() {
    local exit_code=$?
    [[ -z "$_ccr_shim_last_cmd" ]] && return 0
    local now
    now="$(date +%s.%N 2>/dev/null || date +%s)"
    local cmd_escaped
    cmd_escaped="$(_ccr_shim_json_escape "$_ccr_shim_last_cmd")"
    local sid_escaped
    sid_escaped="$(_ccr_shim_json_escape "${WARP_SESSION_ID:-}")"
    _ccr_shim_send "{\"type\":\"command_finished\",\"command\":\"$cmd_escaped\",\"exit_code\":$exit_code,\"warp_session_id\":\"$sid_escaped\",\"started_at\":\"$_ccr_shim_preexec_ts\",\"ts\":\"$now\"}"
    _ccr_shim_last_cmd=""
    _ccr_shim_preexec_ts=""
}
...
if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook preexec _ccr_shim_preexec
    add-zsh-hook precmd _ccr_shim_precmd
fi
